...
class Order(models.Model):
    ...
    loyalty_discount = models.DecimalField(max_digits=10, decimal_places=2,
                                           null=False, default=0)
    deal_discount = models.DecimalField(max_digits=10, decimal_places=2,
                                        null=False, default=0)
...
class OrderLineItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product.category.has_sizes:
            if self.product.is_original:
                price_object = Price.objects.filter(is_premium=product_premium,
                                                    category__name=product_category,
                                                    size=self.product_size).values('price')
            ...
            price_object = Price.objects.filter(is_premium=product_premium,
                                                category__name=product_category).values('price')
            for e in price_object:
                price = e['price']
            self.lineitem_total = price * self.quantity
