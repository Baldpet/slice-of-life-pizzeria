    ...
    try:
        if size:
            ...
            if original_size == size:
                messages.success(request, f'Amended your order of {product.name} - {dough} - {size} to a quantity of {quantity}.')
            else:
                messages.success(request, f'Amended your order to {product.name} - {dough} - {size} and a quantity of {quantity}.')
        else:
...
def remove_item_from_bag(request, item_id):
    ...
    try:
        ...
        if size:
            messages.success(request, f'Removed {product.name} - {dough} - {size} from your order.')
...
def add_pizza_to_bag(request, item_id):
    ...
    try:
        if item_id in list(bag.keys()):
            messages.success(request, f'Added {product.name} - {dough_base} - {size} to your order.')
